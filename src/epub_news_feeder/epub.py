...
class EditionInput:
    ...
    # Which route produced the summaries, so the reader is told whether their reading left
    # the device. Defaults to local: the disclosure must never understate what happened.
...
def _about_ai_document(edition: EditionInput) -> bytes:
    ...
    paragraph.text = _localized(
        edition.language,
        "ai_method_remote" if edition.editorial_route == "remote" else "ai_method_local",
    )
...
_ENGLISH_LABELS = {
    ...
    "ai_method_local": (
        "AI summaries are generated on this device from cited publisher reporting and "
        "independently checked by a second local model. Citation links identify the "
        "reporting used for each sentence."
    ),
    "ai_method_remote": (
        "AI summaries are generated by a remote provider from cited publisher reporting, "
        "and independently checked by a second model. Only reporting whose publisher "
...
_SWEDISH_LABELS = {
    ...
    "ai_method_local": (
        "AI-sammanfattningar skapas på den här enheten från citerad publicistisk text och "
        "granskas oberoende av en andra lokal modell. Citatlänkarna visar vilket underlag "
        "som användes för varje mening."
    ),
    "ai_method_remote": (
        "AI-sammanfattningar skapas hos en extern leverantör från citerad publicistisk text "
        "och granskas oberoende av en andra modell. Endast rapportering vars publicist "
        "tillåter extern behandling skickas dit. Citatlänkarna visar vilket underlag som "
